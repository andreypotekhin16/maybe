...
class CompanyProfile(models.Model):
    ...
    # === ДОБАВЛЕННЫЙ МЕХАНИЗМ АВТО-СОЗДАНИЯ СЕКЦИЙ ===
    def save(self, *args, **kwargs):
        # Сначала сохраняем сам объект CompanyProfile, чтобы у него появился ID
        super().save(*args, **kwargs)
        
        # Список всех секций, которые должны существовать
        ALL_SECTIONS = [
        ...
        # Проходим по списку и создаем секции, если их еще нет
...
class Section(models.Model):
    ...
    class Meta:
        ...
        ordering = ['order']
        # ДОБАВЛЯЕМ УНИКАЛЬНОСТЬ ПО ДВУМ ПОЛЯМ
